...
-- the same reason: a file a program rewrites does not belong in a repo, and
-- this one especially, since the block above is hand-written and commented
-- and a generator that round-tripped it would flatten MONITOR_MAIN back into
-- a literal and take the note at the top of this section with it.
--
-- WHY THIS WORKS AT ALL: a later hl.monitor for the same output replaces the
-- earlier one. Measured before relying on it -- the secondary panel, already
-- at 165 Hz from the block above, moved to 120 the instant a second call
-- named it. So this file is a pure override layer and never has to know what
-- it is overriding.
--
-- pcall + dofile, exactly like the palette below: no file on a fresh clone is
-- the normal case, and a broken one leaves the hand-written values standing
-- rather than taking the whole config down. Delete it and the monitors go
-- back to what this file says.
