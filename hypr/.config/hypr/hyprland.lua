...
-- How thick the border around a window is, from
-- ~/.local/state/hypr-border, written by the `hypr-border` script that the
-- settings window calls. Validated rather than trusted: a mangled file falls
-- back to the 2 this config has always used instead of handing hyprlang
-- something it will refuse.
local function borderSize()
    local home = os.getenv("HOME") or ""
    local dir = os.getenv("XDG_STATE_HOME") or (home .. "/.local/state")
    local file = io.open(dir .. "/hypr-border", "r")
    ...
    if not number or number < 0 or number > 6 then return 2 end
...
hl.config({
    general = {
        ...
        -- From ~/.local/state/hypr-border when the settings window has been
        -- there, 2 otherwise -- the same arrangement desktopOpacity() further
        -- down uses, and for the same reason: `hyprctl eval` puts the value
        -- into the running compositor immediately, and this is what survives
        -- the reload that throws that away.
