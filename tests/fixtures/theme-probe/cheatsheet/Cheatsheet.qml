// The keybind cheatsheet, as the probe draws it: one flat rectangle over the whole screen
// with a word on it, shown while CheatsheetState.isOpen and gone otherwise.
//
...
PanelWindow {
    ...
    // IT DOES NOT ASK THE COMPOSITOR FOR ITS BINDS. genesis calls
    // `Compositor.refreshBinds()` when the sheet opens, and only behind
    // `if (Compositor.can("bindsIntrospection"))`. Unguarded, that spawns a
    // doomed process under a stub compositor -- which is exactly what a
    // headless check runs. A theme that lists no binds does not need the call.
    ...
    Component.onCompleted: console.log("theme-probe drew cheatsheet/Cheatsheet.qml")
}
